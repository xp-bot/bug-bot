...
const description = `Set the ticket archive.`;
const support = false;
const hideHelp = true;
...
async function execute(message, args, member, channel, guild) {
    ...
    SetupConfig.archive_category = cat.id;
    channel.send(new Discord.MessageEmbed().setDescription(`Successfully set the Ticket Archive Category to ${cat}.`).setColor(`#52D94F`).setTitle(`Category Set`))
}